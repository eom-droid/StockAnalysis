...
분기_df = pd.read_csv('./data/분기_df_with_업종.csv')
...
옥수치2009_over_10_per = 분기_df[분기_df['옥수치20/09'] >= 8].copy()

옥수치_주가_corr_df = pd.read_csv('./data/옥수치_주가_corr_df_분기.csv').drop('Unnamed: 0',axis = 1)
...
ccc = list(set(옥수치_10_per) & set(corr_회사이름))
print(ccc)
for i in ccc:
